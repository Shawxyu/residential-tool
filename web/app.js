...
function toast(msg, kind) {
  ...
  d.style.cssText = `position:fixed;bottom:24px;left:50%;transform:translateX(-50%);
    z-index:9999;padding:10px 20px;font-size:12.5px;font-weight:600;
    background:${kind === "err" ? "#a8412f" : "#2a7568"};color:#fff;
    border:1px solid ${kind === "err" ? "#8a3325" : "#16403a"};`;
...
const TILE_PROVIDERS = {
  ...
  "carto-voyager": { url: "https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}.png",
    opts: { maxZoom: MAP_MAX_ZOOM, subdomains: "abcd",
            attribution: "© OpenStreetMap © CARTO" } },
...
/* 降级顺序：本地 → 在线浅色 → 在线彩色 → OSM → 纯矢量 */
const BASEMAP_ORDER = ["local", "carto-light", "carto-voyager", "osm", "none"];
const BASEMAP_LABEL = { local: "本地矢量（离线）", "carto-light": "清新浅色（在线·可能失效）",
  "carto-voyager": "柔和彩色（在线·可能失效）", osm: "OSM 街道（在线）", none: "无底图" };
...
function renderMetrics() {
  ...
  // 表格重构：状态/层数补全收成名称下方的小徽章，说明收进「点击展开」的详情行，
  // 消灭横向滚动和「说明」列撑出的超高行。
...
function addCondition(field = "FAR", op = ">", v = 3.2, v2 = "") {
  ...
  const sync = () => { cv2.style.display = cop.value === "between" ? "" : "none"; };
